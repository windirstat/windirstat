...
void CCheckForUpdateDlg::UpdateFromUrl(CString server, CString uri, INTERNET_PORT port)
{
    ...
    try
    {
        ...
        WDSagent.Format(_T("WinDirStat/%d.%d.%d [Build %d] (%s%4.4X)%s"), VN_MAJOR, VN_MINOR, VN_REVISION, VN_BUILD, _T(UASPEC), GetWDSApp()->GetLangid(), _T(DRSPEC));
        // Create a session
        ...
        {
            CString Response;
            Response.Format(_T("Unexpected HTTP status code: %d"), dwStatusCode);
            DescriptiveText.SetWindowText(Response);
        }
